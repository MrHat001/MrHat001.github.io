...
app.ExplosionSprite = function(){
	...
	function ExplosionSprite(image,width,height,frameWidth,frameHeight,frameDelay) {
		...
		this.numCols = 3 //Math.floor(this.image.width/this.frameWidth);
		this.numRows = 4 //Math.floor(this.image.height/this.frameHeight);
  	...
  	p.draw = function(ctx) {
		...
		if(!this.image){
			...
			ctx.drawImage(
				...
				this.x, this.y, this.width - halfW, this.height - halfH
			);
		}
